...
dt = datetime.now()
print('Comienzo de la creación masiva de prototipos con 20 anidamientos: ', dt)
for i in range(20):
    shopkeeper_clone = shopkeeper_template.clone()
    warrior_clone = warrior_template.clone()
    mage_clone = mage_template.clone()
    for j in range(5):
        shopkeeper_clone = shopkeeper_clone.clone()
        warrior_clone = warrior_clone.clone()
        mage_clone = mage_clone.clone()
        time.sleep(2)  # Simulando carga de procesamiento de 2 segundos
        dt = datetime.now()
        print(f'Finaliza la creación de prototipos en el anidamiento {i+1}-{j+1} at: ', dt)
dt = datetime.now()
print('Finalizó la creación masiva de prototipos: ', dt)
